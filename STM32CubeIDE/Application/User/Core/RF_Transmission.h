...
#ifndef APPLICATION_USER_CORE_RF_TRANSMISSION_H_
#define APPLICATION_USER_CORE_RF_TRANSMISSION_H_
...
static void LED_Process(char* LED_State);

void Update_State(char* payload);

/* USER CODE END PFP */

/* Exported functions ---------------------------------------------------------*/
//SubghzApp_Init
void RF_Transceiver_Init(void)
{
...
#error "Please define a modulation in the subghz_phy_app.h file."
  ...
  UTIL_SEQ_RegTask((1 << CFG_SEQ_Task_LED_Process), UTIL_SEQ_RFU, LED_Process);
...
void Update_State(char *payload)
{
	if (led_state!=payload)
	{
		led_state = payload;
		UTIL_SEQ_SetTask((1 << CFG_SEQ_Task_LED_Process), CFG_SEQ_Prio_0);
	}

}

void LED_Process(char* LED_State)
{
	memcpy(BufferTx, led_state, sizeof(led_state) - 1);
    ...
    case RX:
      ...
      if (isMaster == true)
      {
        if (RxBufferSize > 0)
        {
          ...
          else if (strncmp((const char *)BufferRx, ON, sizeof(ON) - 1) == 0)
          {
            /* A master already exists then become a slave */
          ...
          else if (strncmp((const char *)BufferRx, OFF, sizeof(OFF) - 1) == 0)
          {
            /* A master already exists then become a slave */
...
#endif /* APPLICATION_USER_CORE_RF_TRANSMISSION_H_ */
